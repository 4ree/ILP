...
# ds = datasets[1]
ds_container_path = "./datasets/foodmart.jld"

if isfile(ds_container_path)
    ds = load(ds_container_path)["Dataset"]
else
    # ds = initDataset("foodmart",0, nothing, [0.061,0.062,0.063, 0.064], [0.5, 0.6 , 0.7, 0.8])
    ds = initDataset("foodmart",0, nothing, [0.071,0.072,0.073, 0.074], [0.5, 0.6 , 0.7, 0.8])
...
hide_sensitive_data(als, ds, performances,2)
